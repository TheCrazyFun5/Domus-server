...
class userService {
    async registration(login, pass, role = "user", mail = null) {
        const check = await User.findOne({ where: { login: login } });
        if (check)
            throw errorApi.badRequest(`Пользователь с логином ${login} существует`);
    ...
    async updatAaccessToken(refreshToken) {
