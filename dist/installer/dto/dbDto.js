...
const DMSname = ["mysql", "postgres", "mariadb", "oracle", "mssql"];
const ipRegex = /^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/;
export default function dbDto(data) {
    let error = [];
    if (!data)
        throw errorApi.badRequest("Нет данных");
    if (!data.dms || !DMSname.includes(data.dms))
        error.push(`DMS может равняться: ${DMSname}`);
    if (!data.ip || typeof data.ip != "string" || !ipRegex.test(data.ip))
        error.push(`Неверный IP`);
